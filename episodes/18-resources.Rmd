...
```{r, child=paste(snippets('resources/account-history.Rmd'), sep=''), eval=TRUE}
```
...
```bash
`r config$sched$comment` `r config$sched$flag$time` `r if(!is.null(config$sched$name) && config$sched$name == "Slurm") {print(" ")} else {print("=")}`00:02:00
```
