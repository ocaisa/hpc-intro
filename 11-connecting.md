...
access. For systems like `castle` running an SSH server, anybody
...
```bash
luigi@mushroomkingdom:~$ ssh-keygen -a 100 -f ~/.ssh/id_ed25519 -t ed25519
```
...
```bash
luigi@mushroomkingdom:~$ ssh-keygen -a 100 -b 4096 -f ~/.ssh/id_rsa -o -t rsa
```
...
```bash
luigi@mushroomkingdom:~$ ssh-add -l
```
...
- If you get an error like this one,
  ...
  ```bash
  luigi@mushroomkingdom:~$ eval $(ssh-agent)
  ```
  ...
  ```bash
  luigi@mushroomkingdom:~$ ssh-agent
  ```
...
```bash
luigi@mushroomkingdom:~$ ssh-add -t 8h ~/.ssh/id_ed25519
```
...
Visit [https://pipe.bowser.org](https://pipe.bowser.org) to upload your SSH public key. (Remember, it's the one ending in `.pub`!)

## Log In to the Cluster

Go ahead and open your terminal or graphical SSH client, then log in to the
cluster. Replace `luigi` with your username or the one
...
- `luigi@mushroomkingdom:~$` when the command is to be entered on a terminal
  connected to your local computer
- `luigi@castle:~$` when the command is to be entered on a
...
reference to its parent (`/darkland`). You may or may not see
...
```bash
luigi@castle:~$ cat ~/id_ed25519.pub >> ~/.ssh/authorized_keys
```
