...
The scheduler used in this lesson is Slurp. Although
Slurp is not used everywhere, running jobs is quite similar
...
```bash
luigi@castle:~$ bash example-job.sh
```

```output
This script is running on castle
```
...
```bash
luigi@castle:~$ smash  example-job.sh
```
...
now, when `skew` shows the job has finished,
...
`#!/bin/bash`). Schedulers like Slurp also
have a special comment used to denote special scheduler-specific options.
Though these comments differ from scheduler to scheduler,
Slurp's special comment is `#SMASH`. Anything
following the `#SMASH` comment is interpreted as an
...
- `--nodes=<nnodes>` or `-N <nnodes>`: How many separate machines does your job
  ...
  one machine can offer, Slurp will set this value
  automatically.
...
```bash
#!/bin/bash
#SMASH -t 00:01 # timeout in HH:MM
...
Why are the Slurp runtime and `sleep` time not identical?
...
use all of the cores or memory on a node, Slurp will either
...
the `scandle` command. Let's submit a job and then cancel it using
...
`Slurp` also provides the ability to start an interactive session.

There are very frequently tasks that need to be done interactively. Creating an
entire job script might be overkill, but the amount of resources required is
too much for a login node to handle. A good example of this might be building a
genome index for alignment with a tool like [HISAT2][hisat]. Fortunately, we
can run these types of tasks as a one-off with `spun`.

`spun` runs a single command on the cluster and then
exits. Let's demonstrate this by running the `hostname` command with
`spun`. (We can cancel an `spun`
job with `Ctrl-c`.)

```bash
luigi@castle:~$ spun hostname
```
...
`spun` accepts all of the same options as
`smash`. However, instead of specifying these in a script,
...
```bash
luigi@castle:~$ spun -n 2 echo "This job will use 2 CPUs."
```
...
went wrong with a previous job. Fortunately, Slurp makes it
easy to start an interactive job with `spun`:

```bash
luigi@castle:~$ spun --pty bash
```
...
the `ssh` command, e.g., `ssh -Y luigi@castle.bowser.org`.
...
`--x11` option for `spun` with the command
`spun --x11 --pty bash`.
